 ...
 */
export function collapseVerdict(f: BoxFacts): Verdict {
  if (KEEP.has(f.tagName)) return 'skip';

  // Geometry. Too small to be a visible gap, or large enough to be a page-level
  // container rather than an ad slot.
  if (f.height < 40 || f.width < 100) return 'skip';
  if (f.height > 1400) return 'skip';

  // Shows something, so it is not an emptied box.
  if (f.textLength > 2) return 'skip';
  if (f.hasFormOrMedia) return 'skip';
  ...
  if (f.subtreeCount > 20) return 'skip';
  if (f.hasAnchor) return 'skip';
  if (f.hasAriaOrRole) return 'skip';
  if (f.hasVisibleMedia) return 'skip';
  ...
  if (!f.hasWinnowerHiddenDescendant) return 'skip';
  ...
  return f.seenBefore ? 'collapse' : 'candidate';
}
